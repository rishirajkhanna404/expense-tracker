...
description: Designs and generates modern, production-ready UI for spendwise, a personal expense tracker built on Flask + Jinja2 + vanilla CSS (repo - https://github.com/rishirajkhanna404/expense-tracker). Produces clean fintech-style pages and components - cards, forms, tables, dashboards, modals - with consistent spacing, soft shadows, rounded corners, and Lucide icons. Use this skill whenever the user asks to design, build, create, redesign, improve, or style any spendwise page, screen, section, or component - including phrasings like "design the X page", "create UI for X", "build a component for X", "make the X look better", "redesign X", or any request about spendwise's frontend, layout, CSS, or visual polish - even when spendwise isn't named explicitly if the conversation context is clearly about it.
disable-model-invocation: true
---

# spendwise UI Designer

You are designing frontend UI for **spendwise**, a personal expense tracker. spendwise is a Flask app with server-rendered Jinja2 templates, vanilla CSS, and a sprinkle of vanilla JS. The goal of this skill is to help you generate UI that feels like it belongs in a polished, modern fintech product - not generic bootstrap-era output, and not React/Tailwind output that doesn't match the stack.

## What spendwise's stack looks like
...
If the user's project files are available (e.g. they've shared the repo, uploaded files, or you're inside the codebase), open `base.html`, the main CSS file, and one or two existing templates before generating anything new. The goal is *consistency* - spendwise should feel like one coherent product, not a collage.
...
Pick icons that carry meaning. A few spendwise-appropriate defaults:
